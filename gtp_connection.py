...
class GtpConnection:
    ...
    def policy_cmd(self, args):
        if len(args) != 0:
            ...
            self.go_engine.setPolicy(policytype)
            self.respond()
        else:
            self.respond()
            return
    ...
    def policy_moves_cmd(self, args):
        movelist, isRandomPolicy = self.go_engine.getMovelist(self.board, self.board.current_player)
        
        moves = []
        probabilities = []

        if isRandomPolicy == False:
            movelist = self.go_engine.patternProbability(self.board, self.board.current_player)
            for i in range(len(movelist)):
                moves.append(str(movelist[i][0].lower()))
                probabilities.append(str(movelist[i][1]))

            combined_list = moves + probabilities
            combined_list_string = ' '.join(combined_list)
            self.respond(combined_list_string)

        else:
            probability = 1/len(movelist)
            probability = round(probability, 3)
            for i in range(len(movelist)):
                moves.append(str(movelist[i]))
    ...
    def selection_cmd(self, args):
        if len(args) != 0:
            selectiontype = args[0].lower()
            self.go_engine.setSelection(selectiontype)
            self.respond()
        else:
            self.respond()
            return
